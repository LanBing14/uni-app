<template>
	<view class="uni-load-more" @click="onClick">
		...
		<view v-if="(iconType==='circle' || iconType==='auto' && platform === 'android') && status === 'loading' && showIcon" class="uni-load-more__img">
			<image class="uni-load-more__img--android" src="/static/img/load-book.png" mode="widthFix"></image>
		</view>
		<view v-else-if="status === 'loading' && showIcon" class="uni-load-more__img">
			<image class="uni-load-more__img--ios" src="/static/img/load-book.png" mode="widthFix"></image>
		</view>
		<!-- #endif -->
		<!-- #ifdef APP-NVUE || H5 -->
