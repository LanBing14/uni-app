...
<script>
	...
	export default {
		...
		methods: {
			...
			onHidePic() {
				this.isShow = false;
				uni.setNavigationBarTitle({title: '个人信息编辑'});
			},
			// 初始化图片
			getImgInfo() {
				uni.getImageInfo({
					...
					success: (res) => {
						...
						}  else if (this.img_height > windowHeight && this.img_width >= this.img_height) { // 当图片宽度大于屏幕宽度并且图片宽度小于图片高度时
							this.img_scaling = this.img_height / windowHeight;
							this.scaleMin = tailorSize / windowHeight;
						} else {
							let count = this.img_width <= this.img_height ? this.img_width : this.img_height;
						...
						// 计算图片居中显示时的偏移量
						this.offsetX = -((this.img_width / this.img_scaling / 2) * this.scale - tailorSize / 2);
						this.offsetY = -((this.img_height / this.img_scaling / 2) * this.scale - tailorSize / 2);
						// 获取新的偏移量
			...
			movableScale(e) {
				...
				newOffsetX = -((this.img_width / this.img_scaling / 2) * this.scale - tailorSize / 2);
				newOffsetY = -((this.img_height / this.img_scaling / 2) * this.scale - tailorSize / 2);
			...
			createImg() {
				...
				ctx.draw(false, () => {
					uni.canvasToTempFilePath({
						// 截取图片尺寸
						destWidth: tailorSize,
						destHeight: tailorSize,
